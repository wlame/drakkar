...
**Why is auth off by default?** No endpoint stops a worker, replays Kafka messages, mutates sinks, or commits offsets, and Drakkar expects to run inside a private contour (VPC, internal cluster network, operator-only ingress) — so the default trades authentication for out-of-the-box ergonomics plus a loud startup warning. Two endpoints are not read-only: the **Message Probe** (`ui.probe_enabled`) executes your handler on pasted input and takes executor slots from live traffic, and **Merge** (`ui.merge_enabled`) writes a `merged-<ts>.db` into `db_dir` that nothing reclaims. Both close independently of `auth_token`. Outside a private contour, treat the warning as a prompt to set `auth_token` before the worker accepts traffic.
4. **Kafka producers are trusted for availability, not correctness.** Drakkar deserializes message payloads via `handler.deserialize_message`. The `kafka.on_parse_error` policy decides what happens to a message that fails to parse: `skip` (the default) leaves `msg.payload=None` and passes the message to your handler anyway; `dlq` writes a `ParseFailurePayload` to the DLQ topic and commits the offset once the write is confirmed; `raise` stops the partition processor with the offset uncommitted, so the message is redelivered after restart. A malicious producer cannot execute code in the worker, but under the default policy it can cause handlers to see unexpected `None` payloads unless your handler validates — set `on_parse_error: dlq` if you would rather quarantine them.
5. **Per-task `env` is redacted before it reaches the recorder.** Handler-written values in `task.env` are sanitized on the way to the recorder DB: names matching `*PASSWORD*`/`*PASSWD*`/`*SECRET*`/`*TOKEN*`/`*KEY*`/`*API_KEY*`/`*CREDENTIAL*`/`*_DSN`/`*AUTH*`/`*PRIVATE*`/`*CERT*`/`*SALT*` become `***`; other URL-shaped values have embedded credentials stripped. Non-matching names pass through, so rename or avoid per-task env for secrets whose names don't trigger a pattern. `ExecutorConfig.env` (framework-level) is **never written** to the recorder at all -- it only reaches the subprocess environment, making it the safer slot for stable credentials. This list is deliberately broader than `env_inherit_deny` (below): an over-matched redaction here just costs `***` in a debug DB, whereas an over-matched inheritance-deny pattern would withhold a variable a subprocess needs.
