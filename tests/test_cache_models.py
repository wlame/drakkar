...
- `_encode` / `_decode` — JSON-only serialization helpers with Pydantic revival support
...
from drakkar.cache import CacheEntry, CacheScope, _decode, _encode
...
def test_encode_size_uses_utf8_byte_count_not_codepoint_count():
...
def test_decode_without_as_type_returns_plain_json(value):
...
def test_decode_dict_stays_a_plain_dict_without_as_type():
...
def test_decode_with_as_type_revives_pydantic_model():
    ...
    encoded = json.dumps({'id': 7, 'name': 'alice'})
    user = _decode(encoded, as_type=User)
    assert isinstance(user, User)
    assert user.id == 7
    assert user.name == 'alice'


def test_decode_with_as_type_roundtrips_encoded_pydantic():
    ...
    encoded, _ = _encode(original)
    decoded = _decode(encoded, as_type=Item)
