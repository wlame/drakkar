...
def test_observe_completion_excludes_zero_duration_and_unlabeled():
    ...
    assert tracker.window_stats(now=100.0)['300']['tasks'] == 0
...
def test_window_stats_slices_by_completion_recency():
    tracker = ThroughputTracker(make_config())
    # 100 cost-units at t=1000 (only in wide windows) and 200 at t=1059.5
    # (in every window).
    ...
    assert stats['30'] == {
        'throughput': pytest.approx(200 / 30, abs=0.001),
        'task_rate': pytest.approx(1 / 30, abs=0.001),
        'tasks': 1,
    }
    assert stats['60'] == {'throughput': 5.0, 'task_rate': pytest.approx(2 / 60, abs=0.001), 'tasks': 2}
    assert stats['300'] == {'throughput': 1.0, 'task_rate': pytest.approx(2 / 300, abs=0.001), 'tasks': 2}
...
def test_completions_evicted_past_the_largest_window():
    tracker = ThroughputTracker(make_config())
    tracker.observe_completion({'file_size_bytes': '100'}, 1.0, now=1000.0)
    stats = tracker.window_stats(now=1301.0)  # 301 s later
    assert stats['300']['tasks'] == 0
